...
Each column of these matrices is represented by an arrow in the figure.  In fact, each column of $B_{red}$ can be represented by an integer combination of the columns of $B_{green}$, and vice-versa. If you'd like an exercise, find the matrix $T$ such that $B_{red} =B_{green} T$; this matrix $T$ is a [unimodular matrix](https://en.wikipedia.org/wiki/Unimodular_matrix).  If you don't have a background in linear algebra, or you otherwise got a brain fog from the matrices above, you're in luck, we only have one more matrix to show, and it is one that is intentionally obscure and (essentially) impossible to visualize.
